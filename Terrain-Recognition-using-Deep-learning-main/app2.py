...
# Register FixedDropout layer for serialization
...
# Register swish function for serialization
tf.keras.utils.get_custom_objects()['swish'] = swish

# Load your EfficientNet model
model = tf.keras.models.load_model('C:\\Users\\DIGVIJAY\\OneDrive\\Desktop\\Projects\\Terrain\\efficient_Net.h5')
# Assuming `model` is the loaded model

# Define function to load and preprocess image
def preprocess_image(image_path):
    img = image.load_img(image_path, target_size=(256, 256))  # EfficientNet typically uses input size (224, 224)
...
def select_image():
    ...
    if file_path:
        ...
        img = img.resize((224, 224))  # Resize to match EfficientNet input size
